...
if( ! window.ljs ){
    ...
    ,appendElmt = function(type,attrs,cb){
        ...
        ,loader  = {
            ...
            // Add bulk load with loading bar callback
            ,_all: function(urls, index, callback, loadingBar) {
                loader.load(urls[index], function(){
                    if (index >= urls.length -1) {
                        ...
                        index++;
                        if (loadingBar != undefined || loadingBar != null) {
                            loadingBar.inc(100 / urls.length);
                        }
                        loader._all(urls, index, callback, loadingBar);
                    }
                });
            }
            ,all: function(urls, loadingBar, callback) {
                loader._all(urls, 0, callback, loadingBar);
            }
        }
        ;
        if( checkLoaded ){
